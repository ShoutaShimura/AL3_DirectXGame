...
void GameScene::Initialize() {
	...
	worldTransform_[PartId::LegL].translation_ = { -4.5f,-3.0f,0 };
	worldTransform_[PartId::LegL].parent_ = &worldTransform_[PartId::Hip];
	worldTransform_[PartId::LegL].Initialize();

	worldTransform_[PartId::LegR].translation_ = { 4.5f,-3.0f,0 };
...
//デバック用表示
	...
	//キャラクターの移動処理
	{
		...
		//キャラクターの移動速さ
		const float kCharactearSpeed = 0.2f;

		//押した方向で移動ベクトルを変更
		if (input_->PushKey(DIK_LEFT)) {
			move = { -kCharactearSpeed,0,0 };
		}
		else if (input_->PushKey(DIK_RIGHT)) {
			move = { kCharactearSpeed,0,0 };
		}
	...
	{
		//上半身の回転速さ[ラジアン/frame]
		const float kCheastRotSpeed = 0.05f;

		//押した方向で移動ベクトルを変更
		if (input_->PushKey(DIK_U)) {
			worldTransform_[PartId::Chest].rotation_.y -= kCheastRotSpeed;
		}
